...
class ProdutoService {
  ...
  async criarProduto(dados, imagem, confeiteiroId) {
    ...
    // ✅ Construir FormData conforme esperado pelo @PostMapping do backend
    const formData = new FormData();

    // Limpeza para enviar estritamente o que o ProdutoDTO espera no Java
    const produtoDTO = {
      nome: dados.nome,
    ...
    const url = `${API_ENDPOINTS.PRODUTO.LIST}?confeiteiroId=${confeiteiroId}`;
    ...
    // ✅ POST limpando headers para garantir que o ApiService não force application/json no multipart
